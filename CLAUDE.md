...
A TypeScript monorepo providing a unified abstraction layer for multiple blockchain technologies (ATIPICIAL, Ethereum, Solana, Bitcoin, Stellar, Atipicial X, etc.). All blockchains implement a single `IBlockchainService<N>` interface defined in `@atipicial/blockchain-service`.
...
| `bs-atipicial` | ATC 3 blockchain |
| `bs-atipicial-legacy` | ATC 2 (legacy) blockchain |
| `bs-atipicialx` | Atipicial X blockchain |
| `bs-solana` | Solana |
| `bs-stellar` | Stellar |
| `bs-bitcoin` | Bitcoin |
...
- `IBSWithClaim<N>` — token claim (ATC-specific)
- `IBSWithNft` — NFT data
