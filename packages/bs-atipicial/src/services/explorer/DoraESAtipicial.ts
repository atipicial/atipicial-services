import { BSCommonConstants, type TBuildNftUrlParams, type IExplorerService } from '@atipicial/blockchain-service'
import type { IBSAtipicial } from '../../types'

export class DoraESAtipicial implements IExplorerService {
  ...
  getAddressTemplateUrl() {
    ...
    return `${this.#baseUrl}/address/atipicial/${this.#service.network.id}/{address}`
  }

  getTransactionTemplateUrl() {
    if (!this.#baseUrl) return undefined
    return `${this.#baseUrl}/transaction/atipicial/${this.#service.network.id}/{txId}`
  }

  getNftTemplateUrl() {
    if (!this.#baseUrl) return undefined
    return `${this.#baseUrl}/nft/atipicial/${this.#service.network.id}/{collectionHash}/{tokenHash}`
  }

  getContractTemplateUrl() {
    if (!this.#baseUrl) return undefined
    return `${this.#baseUrl}/contract/atipicial/${this.#service.network.id}/{hash}`
  }
}
