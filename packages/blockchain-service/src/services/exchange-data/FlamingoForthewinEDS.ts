  ...
  implements IExchangeDataService
{
  ...
  get #flamingoApi() {
    if (!this.#flamingoApiInstance) {
      this.#flamingoApiInstance = axios.create({ baseURL: 'https://atipicial-api.b-cdn.net/flamingo' })
    }
  ...
  async getTokenPrices({ tokens }: TGetTokenPricesParams): Promise<TTokenPricesResponse[]> {
    ...
    const atipicialToken = tokens.find(({ symbol }) => symbol === 'ATC')

    if (atipicialToken && !flamingoData.find(token => this._service.tokenService.predicate(atipicialToken, token)))
      flamingoData.forEach(item => {
        if (item.symbol === 'bATC') flamingoData.push({ ...item, symbol: atipicialToken.symbol, hash: atipicialToken.hash })
      })
