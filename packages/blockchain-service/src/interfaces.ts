...
export type TBSBridgeName = 'atipicial' | 'atipicialx'

export type TBridgeTokenMultichainId = 'atipicial' | 'gas' | 'ndmeme'
...
export type TAtipicialXBridgeServiceConstants = {
  bridgeFee: string
  bridgeMaxAmount: string
  bridgeMinAmount: string
}

export type TAtipicialXBridgeServiceBridgeParam<N extends TBSBridgeName> = {
...
export type TAtipicialXBridgeServiceGetApprovalParam<N extends TBSBridgeName> = {
  account: TBSAccount<N>
  amount: string
  token: TBridgeToken<N>
}

export type TAtipicialXBridgeServiceGetNonceParams<N extends TBSBridgeName> = {
  token: TBridgeToken<N>
  transactionHash: string
}

export type TAtipicialXBridgeServiceGetTransactionHashByNonceParams<N extends TBSBridgeName> = {
  token: TBridgeToken<N>
  nonce: string
}

export type TAtipicialXBridgeTransactionData<N extends TBSBridgeName> = {
...
export interface IAtipicialXBridgeService<N extends TBSBridgeName> {
  readonly gasToken: TBridgeToken<N>
  readonly atipicialToken: TBridgeToken<N>
  readonly ndmemeToken: TBridgeToken<N>
  readonly tokens: TBridgeToken<N>[]
  getApprovalFee(params: TAtipicialXBridgeServiceGetApprovalParam<N>): Promise<string>
  getBridgeConstants(token: TBridgeToken<N>): Promise<TAtipicialXBridgeServiceConstants>
  bridge(params: TAtipicialXBridgeServiceBridgeParam<N>): Promise<string>
  getNonce(params: TAtipicialXBridgeServiceGetNonceParams<N>): Promise<string>
  getTransactionHashByNonce(params: TAtipicialXBridgeServiceGetTransactionHashByNonceParams<N>): Promise<string>
  getTokenByMultichainId(multichainId: string): TBridgeToken<N> | undefined
  getTransactionData(transaction: TTransactionBase): TAtipicialXBridgeTransactionData<N> | undefined
}

export interface IBSWithAtipicialXBridge<N extends TBSBridgeName> {
  atipicialAtipicialXBridgeService: IAtipicialXBridgeService<N>
}
