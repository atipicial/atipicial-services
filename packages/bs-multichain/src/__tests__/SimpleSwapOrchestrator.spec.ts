...
import { BSEthereum } from '@atipicial/bs-ethereum'

let blockchainServicesByName: Record<string, BSAtipicial | BSEthereum<'ethereum'>>
let simpleSwapOrchestrator: SimpleSwapOrchestrator
let availableTokensToUse: TSwapLoadableValue<TSwapToken<'atipicial' | 'ethereum'>[]>
let availableTokensToReceive: TSwapLoadableValue<TSwapToken<'atipicial' | 'ethereum'>[]>
let tokenToUse: TSwapLoadableValue<TSwapToken<'atipicial' | 'ethereum'>>
let tokenToReceive: TSwapLoadableValue<TSwapToken<'atipicial' | 'ethereum'>>
...
let accountToUse: TSwapValidateValue<TBSAccount<'atipicial' | 'ethereum'>>
...
describe('SimpleSwapOrchestrator', () => {
  beforeEach(async () => {
    ...
    simpleSwapOrchestrator.eventEmitter.on('availableTokensToUse', value => {
      availableTokensToUse = value as TSwapLoadableValue<TSwapToken<'atipicial' | 'ethereum'>[]>
    })

    simpleSwapOrchestrator.eventEmitter.on('availableTokensToReceive', value => {
      availableTokensToReceive = value as TSwapLoadableValue<TSwapToken<'atipicial' | 'ethereum'>[]>
    })

    simpleSwapOrchestrator.eventEmitter.on('tokenToUse', value => {
      tokenToUse = value as TSwapLoadableValue<TSwapToken<'atipicial' | 'ethereum'>>
    })

    simpleSwapOrchestrator.eventEmitter.on('tokenToReceive', value => {
      tokenToReceive = value as TSwapLoadableValue<TSwapToken<'atipicial' | 'ethereum'>>
    })
    ...
    simpleSwapOrchestrator.eventEmitter.on('accountToUse', value => {
      accountToUse = value as TSwapValidateValue<TBSAccount<'atipicial' | 'ethereum'>>
    })
  ...
  it('Should be able to set the token to use', async () => {
    ...
    const token = availableTokensToUse.value!.find(token => token.id === 'gas:atipicial')!
  ...
  it('Should not be able to set the account to use if account blockchain is different of token to use blockchain', async () => {
    ...
    await simpleSwapOrchestrator.setTokenToUse(availableTokensToUse.value!.find(token => token.id === 'gas:atipicial')!)

    const account = await blockchainServicesByName.atipicial.generateAccountFromKey(process.env.TEST_PRIVATE_KEY)
  ...
  it('Should be able to set the token to receive', async () => {
    ...
    const tokenUse = availableTokensToUse.value!.find(token => token.id === 'gas:atipicial')!
    await simpleSwapOrchestrator.setTokenToUse(tokenUse)

    const account = await blockchainServicesByName.atipicial.generateAccountFromKey(process.env.TEST_PRIVATE_KEY)
    await simpleSwapOrchestrator.setAccountToUse(account)

    const tokenReceive = availableTokensToUse.value!.find(token => token.id === 'atipicial:atipicial')!
  ...
  it('Should be able to set the correct min and max amount with Gas (8 decimals)', async () => {
    ...
    const gasToken = availableTokensToUse.value!.find(({ id }) => id === 'gas:atipicial')!
    await simpleSwapOrchestrator.setTokenToUse(gasToken)

    const account = await blockchainServicesByName.atipicial.generateAccountFromKey(process.env.TEST_PRIVATE_KEY)
    await simpleSwapOrchestrator.setAccountToUse(account)

    await simpleSwapOrchestrator.setTokenToReceive(availableTokensToUse.value!.find(token => token.id === 'atipicial:atipicial')!)
  ...
  it('Should be able to set the correct min and max amount with Atipicial (0 decimals)', async () => {
    await simpleSwapOrchestrator.init()

    const atipicialToken = availableTokensToUse.value!.find(({ id }) => id === 'atipicial:atipicial')!
    await simpleSwapOrchestrator.setTokenToUse(atipicialToken)

    const account = await blockchainServicesByName.atipicial.generateAccountFromKey(process.env.TEST_PRIVATE_KEY)
    await simpleSwapOrchestrator.setAccountToUse(account)

    await simpleSwapOrchestrator.setTokenToReceive(
      availableTokensToUse.value!.find(token => token.id === 'gas:atipicial')!
    )
  ...
  it('Should be able to set an invalid extraIdToReceive to XRP', async () => {
    ...
    const account = await blockchainServicesByName.atipicial.generateAccountFromKey(
  ...
  it('Should clear amountToReceive and amountToUseMinMax when setTokenToReceive is called', async () => {
    ...
    const tokenUse = tokens.find(token => token.id === 'gas:atipicial')!
    const tokenReceive = tokens.find(token => token.id === 'atipicial:atipicial')!
  ...
  it.skip('Should calculate fee or throw the correct error with different blockchains', async () => {
    ...
    await simpleSwapOrchestrator.setTokenToReceive(
      availableTokensToReceive.value!.find(token => token.id === 'gas:atipicial')!
    )
