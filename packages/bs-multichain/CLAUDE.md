...
Cross-chain aggregator. Composes multiple blockchain services into a single unified interface and implements multi-chain features: token swaps (SimpleSwap), Atipicial↔AtipicialX bridging, and multi-chain WalletConnect.
...
│   │   └── types.ts
│   ├── bridge/
│   │   ├── AtipicialXBridgeOrchestrator.ts  # State machine for bridge flow
│   │   └── types.ts
│   └── wallet-connect/
...
Both `SimpleSwapOrchestrator` and `AtipicialXBridgeOrchestrator` implement a state machine:
...
`WalletKitHelper` wraps `@walletconnect/utils` and handles session management across all registered blockchain services simultaatipicialusly.
