...
import type { TAtipicialXBridgeOrchestratorInitParams, TAtipicialXBridgeOrchestratorWaitParams } from './types'

export class AtipicialXBridgeOrchestrator implements IBridgeOrchestrator<TBSBridgeName> {
  eventEmitter: TypedEmitter<TBridgeOrchestratorEvents<TBSBridgeName>>

  fromService: BSAtipicial | BSAtipicialX
  toService: BSAtipicial | BSAtipicialX
  ...
  constructor(params: TAtipicialXBridgeOrchestratorInitParams) {
    this.eventEmitter = new EventEmitter() as TypedEmitter<TSwapOrchestratorEvents<TBSBridgeName>>

    const isInitialAtipicialX = params.initialFromServiceName === params.atipicialXService.name

    this.fromService = isInitialAtipicialX ? params.atipicialXService : params.atipicialService
    this.toService = isInitialAtipicialX ? params.atipicialService : params.atipicialXService
  }
  ...
  async init(): Promise<void> {
    this.#availableTokensToUse = {
      value: this.fromService.atipicialAtipicialXBridgeService.tokens,
    }
  ...
  async setTokenToUse(token: TBridgeToken<TBSBridgeName> | null): Promise<void> {
    ...
    try {
      ...
      if (token) {
        ...
        tokenToReceive = this.toService.atipicialAtipicialXBridgeService.getTokenByMultichainId(token.multichainId)
      }
  ...
  async setBalances(balances: TBalanceResponse[] | null): Promise<void> {
    ...
    try {
      const atipicialAtipicialXBridgeService = this.fromService.atipicialAtipicialXBridgeService as IAtipicialXBridgeService<TBSBridgeName>
      const constants = await atipicialAtipicialXBridgeService.getBridgeConstants(this.#tokenToUse.value)
  ...
  async setAmountToUse(amount: string | null): Promise<void> {
    ...
    this.#amountToUseTimeout = setTimeout(async () => {
      ...
      try {
        ...
        const approvalFee = await atipicialAtipicialXBridgeService
          .getApprovalFee({
  ...
  async bridge(): Promise<string> {
    ...
    return await atipicialAtipicialXBridgeService.bridge({
      account: this.#accountToUse.value,
      token: this.#tokenToUse.value,
  ...
  static async wait({
    ...
    atipicialService,
    atipicialXService,
  }: TAtipicialXBridgeOrchestratorWaitParams) {
    const isAtipicialService = tokenToUse.blockchain === atipicialService.name

    const fromService = isAtipicialService ? atipicialService : atipicialXService
    const toService = isAtipicialService ? atipicialXService : atipicialService

    const fromAtipicialXBridgeService = fromService.atipicialAtipicialXBridgeService as IAtipicialXBridgeService<TBSBridgeName>
    const toAtipicialXBridgeService = toService.atipicialAtipicialXBridgeService as IAtipicialXBridgeService<TBSBridgeName>

    const nonce = await BSUtilsHelper.retry(
      () =>
        fromAtipicialXBridgeService.getNonce({
          token: tokenToUse,
          transactionHash,
        }),
      {
    ...
    await BSUtilsHelper.retry(
      () => toAtipicialXBridgeService.getTransactionHashByNonce({ nonce, token: tokenToReceive }),
      {
        retries: 10,
        delay: 30000,
      }
