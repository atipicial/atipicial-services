import { BSError, BSKeychainHelper, type TBSAccount } from '@atipicial/blockchain-service'
...
import { wallet } from '@atipicial/atipicial-core'

let service: BSAtipicial
let walletConnectService: WalletConnectServiceAtipicial
let account: TBSAccount<TBSAtipicialName>

describe('WalletConnectServiceAtipicial', () => {
  beforeEach(async () => {
    service = new BSAtipicial(BSAtipicialConstants.TESTNET_NETWORK)
    walletConnectService = new WalletConnectServiceAtipicial(service)
  ...
  it('Should have correct namespace and chain', () => {
    expect(walletConnectService.namespace).toBe('atipicial')
    expect(walletConnectService.chain).toBe(`atipicial:${service.network.id}`)
  })

  it('Should have correct chain for custom network', () => {
    const customService = new BSAtipicial({
      id: 'custom-id',
      name: 'Custom',
      url: 'http://localhost:50012',
      type: 'custom',
    })
    const customWalletConnectService = new WalletConnectServiceAtipicial(customService)

    expect(customWalletConnectService.chain).toBe('atipicial:private')
  })
  ...
  it('Should be able to process signTransaction and the witness should match the account', async () => {
    ...
    expect(verificationHex).toContain(atipicialJsAccount.publicKey)
  })
  ...
  it('Should be able to process encrypt with explicit public keys', async () => {
    const atipicialJsAccount = new wallet.Account(account.key)
    const publicKey = atipicialJsAccount.publicKey
  ...
  it('Should return isLedger true for hardware account', async () => {
    const hardwareAccount: TBSAccount<TBSAtipicialName> = {
      ...account,
      isHardware: true,
    }
