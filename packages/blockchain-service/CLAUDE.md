...
`IBlockchainService<N extends string, A extends string = string>` is the central contract. `N` is the blockchain name literal (e.g. `'atipicial'`), `A` is the network ID literal (e.g. `'mainnet'`).
...
| `IBSWithClaim<N>` | `isClaimable` | Token claiming (ATC) |
